...
input_el.addEventListener("keyup", HandleUserKeyUp);

function HandleUserKeyUp(ev) {
  ...
  const argsSplit = args.slice(1);

  switch (command) {
    case "echo":
      return EchoHandler(command, argsSplit);
    case "varclear":
      return VarClearHandler(command, argsSplit);
    case "vardump":
      return VarDumpHandler(command, argsSplit);
    case "vardump":
      return VarDumpHandler(command, argsSplit);
    case "set":
      return SetHandler(command, argsSplit);
    case "+":
    case "-":
    case "max":
      return BinaryArithmeticHandler(command, argsSplit);

    default:
      CreateError(`command not found: ${command}.`);
  }
}

function VarClearHandler() {
  state = {};
}
...
function HelpHandler() {
  ...
  const message =
    `[${len}] available commands: ` + Object.keys(handlers).join(", ");
  CreateSuccess(message);
}
...
function EchoHandler(_, args = []) {
  if (args.length !== 1) {
    return CreateError(
      `echo command expects 1 argument. Received ${args.length} instead. ex: 'echo foo'`,
    );
  }
...
function SetHandler(_, args = []) {
  ...
  if (args.length !== 2) {
    return CreateError(
      `set command expects 2 arguments. Only received ${args.length}. ex: 'set x 10'`,
    );
  }
  ...
  if (IsAlphabetic(value)) {
    if (state[value] !== undefined) {
      ...
      return CreateSuccess(
        `${assignee} : ${state[value]} (copied from ${value})`,
      );
    }
  ...
  return CreateError(
    `invalid value: '${value}'. value must be numeric or an existing variable name.`,
  );
}
...
function BinaryArithmeticHandler(cmd, args) {
  if (args.length !== 2) {
    return CreateError(
      `${cmd} command expects 2 arguments. Only received ${args.length}. ex: '${cmd} 10 20'`,
    );
  }
  ...
  if (!IsNumeric(args[0]) || !IsNumeric(1)) {
    return CreateError(
      `One or both arugments to ${cmd} is not numeric. arg1: ${args[0]} & arg2: ${args[1]}`,
    );
  }
